...
const GAME_PATH      = '/games/volcano-tap'
const ACCENT         = '#ef4444'
...
test('4.3 — lava bubbles appear on canvas', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
...
test('5.2 — miss HUD shows 0 initially', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  const missEl = page.locator('text=MISS').locator('..').locator('span, div').last()
  const text = await missEl.textContent().catch(() => '0')
...
test('5.3 — game ends at timer 0', async ({ page }) => {
  ...
  await game.goto()
  await game.start()
  await page.waitForSelector('button:has-text("Play Again")', { timeout: Math.ceil(GAME_DURATION_MS / 10) + 8000 })
...
test('12.1 — haptics fire after tap', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  ...
  if (box) {
    for (let i = 0; i < 10; i++) {
      await page.mouse.click(box.x + Math.random() * box.width, box.y + Math.random() * box.height * 0.7)
      await page.waitForTimeout(100)
    }
  }
  const log = await game.getVibrateLog()
  console.log(`Volcano Tap haptics: ${log.length}`)
})
