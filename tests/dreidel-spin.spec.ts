...
test('4.3 — no crash during dreidel flick gameplay', async ({ page }) => {
  const errors: string[] = []
  page.on('pageerror', err => errors.push(err.message))
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
...
test('5.3 — play-again resets', async ({ page }) => {
  await page.addInitScript(() => {
    const o = window.setInterval.bind(window)
    ;(window as any).setInterval = (fn: () => void, ms: number, ...a: unknown[]) =>
      ms === 1000 ? o(fn, 100, ...a) : o(fn, ms, ...a)
...
test('6.1 — end screen shows dreidel personality', async ({ page }) => {
  await page.addInitScript(() => {
    const o = window.setInterval.bind(window)
    ;(window as any).setInterval = (fn: () => void, ms: number, ...a: unknown[]) =>
      ms === 1000 ? o(fn, 100, ...a) : o(fn, ms, ...a)
  ...
  await game.start()
  await game.waitForEnd(GAME_DURATION_MS / 10 + 5000)
  const p = page.locator('text=/Gimel King|Lucky Spinner|Power Spinner|Shin Magnet|Hanukkah Player/').first()
...
test('9.1 — axe-core start screen', async ({ page }) => {
  ...
  const c = r.violations.filter(v => v.impact === 'critical' || v.impact === 'serious')
