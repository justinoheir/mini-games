...
const GAME_PATH      = '/games/aurora-wave'
const ACCENT         = '#34d399'
...
test('11.1 — fallback simulated breath renders aurora without mic', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto() // no mic permission
  await game.start()
  await game.waitForPlaying()
  await page.waitForTimeout(3000)
  // Should still render (simulated breath fallback)
