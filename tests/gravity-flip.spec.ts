...
const GAME_PATH      = '/games/gravity-flip'
const ACCENT         = '#8b5cf6'
...
test('5.2 — tap flips gravity without crash', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
...
test('12.1 — haptics log after tap', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  ...
  console.log(`Gravity Flip haptics: ${log.length}`)
})
