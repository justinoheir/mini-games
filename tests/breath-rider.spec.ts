 ...
 * Generated by QA Agent | 2026-03-22
 *
 * GAME_ID:   breath-rider
 * SENSOR:    mic (touch fallback)
 * ACCENT:    #3b82f6 (useBrandTheme default blue)
 * DURATION:  45s
 */
...
test('4.4 — canvas is visible during gameplay', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto({ sensors: { mic: true } })
  await game.start()
  await game.waitForPlaying()
...
test('5.5 — end screen shows personality classification', async ({ page }) => {
  ...
  // Personality label must be non-empty — check for known archetypes
  const personality = page.locator('text=/Steady|Focused|Variable/').first()
...
test('9.4 — end screen passes axe-core scan', async ({ page }) => {
  ...
  const critical = results.violations.filter(v => v.impact === 'critical' || v.impact === 'serious')
  expect(critical, `End screen axe violations: ${critical.map(v => v.id).join(', ')}`).toHaveLength(0)
})

// ─── 10. MIC SENSOR SPECIFIC ─────────────────────────────────────────────────

test('10.1 — touch fallback activates when mic is denied', async ({ page }) => {
  // Simulate mic denied
  await page.addInitScript(() => {
  ...
  await game.goto()
  await game.start()
  // Should still proceed to countdown/playing with touch fallback
  await game.waitForPlaying()
  await expect(game.canvas).toBeVisible({ timeout: 3000 })
})

test('10.2 — game starts from silent mic (touch fallback path)', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto({ sensors: { mic: true } })
  await game.mockMicrophone('silent')
  await game.start()
  await game.waitForPlaying()
  // Timer should still count down even without mic input
  await game.expectTimerDecreasing(3000)
})
...
test('11.1 — haptics fire at least once during gameplay', async ({ page }) => {
  ...
  // Haptics may not fire without coin collection; just log without failing
})
