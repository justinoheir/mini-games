/**
 * QA Test Suite — Reflex Rally
 * GAME_ID: reflex-rally
 * SENSOR: touch
 * ACCENT: #0891b2
 * DURATION: 45s
 */
...
test('2.2 — name input is visible on start screen', async ({ page }) => {
  ...
  // May need to dismiss SwipeInstructions first
  const playBtn = page.locator('button:has-text("Play")').first()
  const hasInstructions = await playBtn.isVisible({ timeout: 2000 }).catch(() => false)
  if (hasInstructions) {
    // Click through all instructions
    for (let i = 0; i < 3; i++) {
      const nextBtn = page.locator('button:has-text("Next →"), button:has-text("Play")').first()
