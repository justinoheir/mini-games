...
const GAME_PATH      = '/games/neon-archer'
const ACCENT         = '#00ffcc'
...
test('5.4 — swipe gesture fires arrow on canvas', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
...
test('12.1 — haptics fire during gameplay', async ({ page }) => {
  ...
  console.log(`Neon Archer haptics: ${log.length}`)
})
