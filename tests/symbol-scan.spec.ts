/**
 * Symbol Scan — QA Test Suite
 * Game: Find target symbols in a 4×4 grid before the clock runs out.
 * Sensor: touch | Duration: 60s | Accent: #10b981
 *
 * Run: npx playwright test tests/symbol-scan.spec.ts --headed
 */
...
test('3.2 — countdown progresses to GO then to playing', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await expect(
...
test('4.4 — FOUND score label visible in HUD', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  // HUD should show the FOUND label
  const foundLabel = page.locator('text=FOUND')
  await expect(foundLabel).toBeVisible({ timeout: 3000 })
})
...
test('4.6 — tapping grid cells does not throw errors', async ({ page }) => {
  const errors: string[] = []
  page.on('pageerror', err => errors.push(err.message))
  ...
  // Tap several positions in the grid area (center of screen)
  const vp = page.viewportSize()!
  for (let i = 0; i < 8; i++) {
    const x = vp.width  * 0.25 + Math.random() * vp.width  * 0.5
    const y = vp.height * 0.35 + Math.random() * vp.height * 0.5
    await page.tap({ position: { x: Math.round(x), y: Math.round(y) } }).catch(() => {})
    await page.waitForTimeout(200)
  }

  expect(errors, `Errors after tapping: ${errors.join(', ')}`).toHaveLength(0)
})
...
test('5.4 — timer resets after play-again', async ({ page }) => {
  await page.addInitScript(() => {
    const orig = window.setInterval.bind(window)
    ;(window as any).setInterval = (fn: () => void, ms: number, ...args: unknown[]) => {
      if (ms === 1000) return orig(fn, 100, ...args)
...
test('6.2 — end screen shows personality classification', async ({ page }) => {
  ...
  // Any of the 4 personality types should be visible
  const personality = page.locator(
    'text=/Eagle Eye|Rapid Scanner|Methodical|Pattern Seeker/i'
  ).first()
  await expect(personality).toBeVisible({ timeout: 3000 })
})

test('6.3 — end screen shows all 4 insight chips', async ({ page }) => {
  await page.addInitScript(() => {
    const orig = window.setInterval.bind(window)
    ;(window as any).setInterval = (fn: () => void, ms: number, ...args: unknown[]) => {
      if (ms === 1000) return orig(fn, 100, ...args)
  ...
  // Expect to see all 4 insight labels
  for (const label of ['Targets Found', 'Accuracy', 'Grids Cleared', 'Avg Search']) {
    const el = page.locator(`text=${label}`).first()
    await expect(el).toBeVisible({ timeout: 3000 })
  }
})

test('6.4 — end screen does not require scrolling on iPhone SE', async ({ page }) => {
...
// ─── 10. SYMBOL SCAN SPECIFIC ────────────────────────────────────────────────

test('10.1 — FIND label visible in target area during gameplay', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()

  // The canvas draws "FIND" text — check the canvas is present and full viewport
...
test('10.2 — haptics fire during gameplay interactions', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  ...
  console.log(`Symbol Scan haptics fired: ${log.length} times`)
})

test('10.3 — difficulty test: casual player simulation survives full duration', async ({ page }) => {
  // Simulate below-average player: 500ms response time, taps with 300ms delays
  const errors: string[] = []
  page.on('pageerror', err => errors.push(err.message))
  ...
  const vp = page.viewportSize()!
  const endTime = Date.now() + 55000 // check up to 55s

  // Every 600ms, tap a random grid cell (casual player speed)
  while (Date.now() < endTime) {
    const x = vp.width  * 0.2 + Math.random() * vp.width  * 0.6
    const y = vp.height * 0.35 + Math.random() * vp.height * 0.55
    await page.tap({ position: { x: Math.round(x), y: Math.round(y) } }).catch(() => {})
    await page.waitForTimeout(500 + Math.random() * 400) // 500-900ms response time

    // Check if game ended unexpectedly
    const endScreen = await game.playAgainButton.isVisible().catch(() => false)
    if (endScreen) break
  }

  // Game should NOT have ended before 45 seconds (difficulty fail)
  // (we check if end screen appeared; if it did, it should be due to timer, not forced game over)
  expect(errors, `Crash during casual simulation: ${errors.join(', ')}`).toHaveLength(0)
})
