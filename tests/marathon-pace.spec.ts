...
const GAME_PATH      = '/games/marathon-pace'
const ACCENT         = '#22c55e'
...
test('4.3 — runner animated on canvas', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto({ sensors: { motion: true } })
  await game.start()
  await game.waitForPlaying()
...
test('5.2 — mouse move controls pace', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
...
test('12.1 — haptics fire during zone time', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  ...
  console.log(`Marathon Pace haptics: ${log.length}`)
})
