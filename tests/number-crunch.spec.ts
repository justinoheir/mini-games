...
const GAME_PATH      = '/games/number-crunch'
const ACCENT         = '#3b82f6'
...
test('5.4 — math problem appears in gameplay', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  await page.waitForTimeout(1000)
  // Problem should show a math operator
  const problem = page.locator('text=/\\+|−|×|÷/').first()
  await expect(problem).toBeVisible({ timeout: 3000 })
})

test('5.5 — tapping answer button responds', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  ...
  const firstBtn = page.locator('button[aria-label^="Answer"]').first()
...
test('5.6 — personality shown on end screen', async ({ page }) => {
  await page.addInitScript(() => {
    const orig = window.setInterval.bind(window)
    ;(window as any).setInterval = (fn: () => void, ms: number, ...args: unknown[]) => {
      if (ms === 1000) return orig(fn, 100, ...args)
