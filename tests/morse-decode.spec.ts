...
const GAME_PATH      = '/games/morse-decode'
const ACCENT         = '#facc15'
...
test('4.3 — answer buttons appear during answering phase', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  // Wait for answering phase (after flash)
  const answerBtn = page.locator('button[aria-label^="Answer"]').first()
  await expect(answerBtn).toBeVisible({ timeout: 8000 })
})

test('4.4 — no crash during 10s gameplay', async ({ page }) => {
  const errors: string[] = []
  page.on('pageerror', err => errors.push(err.message))
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
...
test('5.2 — tapping answer button does not crash', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  // Wait for answer phase and tap any button
  const answerBtn = page.locator('button[aria-label^="Answer"]').first()
  await answerBtn.waitFor({ timeout: 8000 }).catch(() => {})
...
test('12.1 — haptics log after answer', async ({ page }) => {
  const game = new GamePage(page, GAME_PATH, ACCENT)
  await game.goto()
  await game.start()
  await game.waitForPlaying()
  const answerBtn = page.locator('button[aria-label^="Answer"]').first()
  await answerBtn.waitFor({ timeout: 8000 }).catch(() => {})
  if (await answerBtn.isVisible()) await answerBtn.click()
  await page.waitForTimeout(1000)
  const log = await game.getVibrateLog()
  console.log(`Morse Decode haptics: ${log.length}`)
})
