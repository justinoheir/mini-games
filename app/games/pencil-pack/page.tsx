...
const GAME_TAGLINE = 'Sort and pack. School starts now.';
const BG_COLOR = '#00071a';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'A+ Student 📚';
  if (acc >= 0.55) return 'Organized 📐';
  if (sig.maxStreak >= 4) return 'Quick Packer ⚡';
  return 'Scattered 🎒';
}
...
export default function PencilPackGame() {
  ...
  const QUESTIONS: {q:string,opts:string[],c:number}[] = [{"q":"Pencil goes in?","opts":["Front pocket","Lunch box","Shoes","Jacket"],"c":0},{"q":"Lunch goes in?","opts":["Book bag","Lunch compartment","Pencil case","Gym bag"],"c":1},{"q":"Books belong in?","opts":["Pencil pouch","Main compartment","Lunch box","Gym bag"],"c":1},{"q":"Water bottle goes in?","opts":["Main pocket","Side pocket","Front zipper","Lunch box"],"c":1}];
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Pencil Pack game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
