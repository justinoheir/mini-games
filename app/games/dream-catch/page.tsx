...
const BG_COLOR = '#070014';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'drive';
const PB_KEY = 'mg_pb_dream-catch';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Dream Weaver 🌙';
  if (acc >= 0.55) return 'Dream Catcher 🌟';
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Dream Catch game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
