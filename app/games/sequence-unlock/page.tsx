...
const GAME_ID='sequence-unlock';const ACCENT='#a855f7';const DURATION=45;const GAME_EMOJI='🔐';const GAME_TITLE='Sequence Unlock';const GAME_TAGLINE='Watch the lights. Repeat them.';
...
const COLORS=['#a855f7','#4ade80','#3b82f6','#f43f5e','#fbbf24','#a855f7'];
...
export default function SequenceUnlock(){
  ...
  return(
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      {phase==='start'&&<GameStartScreen emoji={GAME_EMOJI} title={GAME_TITLE} description="Watch the sequence of lights, then repeat it exactly!" ctaLabel="Play! 🔐" accentColor={theme.colors.accent??ACCENT} onStart={handleStart}/>}
      {phase==='countdown'&&<Countdown onComplete={startLoop} accentColor={theme.colors.accent??ACCENT}/>}
      {(phase==='playing'||phase==='countdown')&&(<><canvas ref={canvasRef} style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}} role="img" aria-label="Sequence Unlock game canvas"/>
