...
const BG_COLOR = '#001a17';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'minimal';
const PB_KEY = 'mg_pb_pattern-predict';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Pattern Oracle 🔮';
  if (acc >= 0.55) return 'Analyst 📈';
  if (sig.maxStreak >= 4) return 'Systematic 📐';
...
export default function PatternPredictGame() {
  ...
  const QUESTIONS: {q:string,opts:string[],c:number}[] = [{"q":"2, 4, 6, 8, ?","opts":["9","10","11","12"],"c":1},{"q":"1, 3, 9, 27, ?","opts":["54","72","81","90"],"c":2},{"q":"A, C, E, G, ?","opts":["H","I","J","K"],"c":1},{"q":"1, 1, 2, 3, 5, ?","opts":["7","8","9","10"],"c":1},{"q":"100, 50, 25, ?","opts":["10","12","12.5","15"],"c":2}];
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Pattern Predict game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
