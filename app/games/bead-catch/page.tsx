...
const GAME_TAGLINE = 'Tilt to catch the beads!';
const BG_COLOR = '#0d0014';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'holiday';
const PB_KEY = 'mg_pb_bead-catch';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Mardi Gras MVP 🎊';
  if (acc >= 0.55) return 'Bead Collector 📿';
  if (sig.maxStreak >= 4) return 'Nimble Catcher 🏃';
  return 'Bead Spiller 😅';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Bead Catch game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
