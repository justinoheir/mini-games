...
const GAME_TAGLINE = 'Find the weak spot. Smash!';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Piñata Pro 🎊';
  if (acc >= 0.55) return 'Party Animal 🎉';
  if (sig.maxStreak >= 4) return 'Strong Arm 💪';
  return 'Blind Bat 🦇';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Piñata Smash game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
