...
const GAME_ID='word-flash';const ACCENT='#ec4899';const DURATION=45;const GAME_EMOJI='⚡';const GAME_TITLE='Word Flash';const GAME_TAGLINE='Read it. Remember it. Recall it.';
...
const COLORS=['#ec4899','#4ade80','#3b82f6','#f43f5e','#fbbf24','#a855f7'];
...
export default function WordFlash(){
  ...
  return(
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      {phase==='start'&&<GameStartScreen emoji={GAME_EMOJI} title={GAME_TITLE} description="Memorize the flashed words then tap them in order!" ctaLabel="Play! ⚡" accentColor={theme.colors.accent??ACCENT} onStart={handleStart}/>}
      {phase==='countdown'&&<Countdown onComplete={startLoop} accentColor={theme.colors.accent??ACCENT}/>}
      {(phase==='playing'||phase==='countdown')&&(<><canvas ref={canvasRef} style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}} role="img" aria-label="Word Flash game canvas"/>
