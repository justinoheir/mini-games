...
const GAME_TAGLINE = 'Planets fall. Catch them all.';
const BG_COLOR = '#07070f';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'drive';
const PB_KEY = 'mg_pb_cosmic-catch';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Astronomer ??';
  if (acc >= 0.55) return 'Star Catcher ?';
  if (sig.maxStreak >= 4) return 'Space Cadet ??';
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Cosmic Catch game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
