...
const GAME_TAGLINE = 'Follow the clues. Find the answer.';
const BG_COLOR = '#0a0014';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'minimal';
const PB_KEY = 'mg_pb_inference-trail';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Sherlock 🔍';
  if (acc >= 0.55) return 'Detective 🕵️';
  if (sig.maxStreak >= 4) return 'Deductive 🧩';
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Inference Trail game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
