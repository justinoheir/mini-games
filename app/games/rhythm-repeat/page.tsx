...
const GAME_ID='rhythm-repeat';const ACCENT='#f59e0b';const DURATION=45;const GAME_EMOJI='🥁';const GAME_TITLE='Rhythm Repeat';const GAME_TAGLINE='Hear the beat. Play it back.';
...
const COLORS=['#f59e0b','#4ade80','#3b82f6','#f43f5e','#fbbf24','#a855f7'];
...
export default function RhythmRepeat(){
  ...
  return(
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      {phase==='start'&&<GameStartScreen emoji={GAME_EMOJI} title={GAME_TITLE} description="Listen to the rhythm pattern and reproduce it by tapping!" ctaLabel="Play! 🥁" accentColor={theme.colors.accent??ACCENT} onStart={handleStart}/>}
      {phase==='countdown'&&<Countdown onComplete={startLoop} accentColor={theme.colors.accent??ACCENT}/>}
      {(phase==='playing'||phase==='countdown')&&(<><canvas ref={canvasRef} style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}} role="img" aria-label="Rhythm Repeat game canvas"/>
