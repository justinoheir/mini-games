...
const GAME_TAGLINE = 'Hear the beat. Play it back.';
const BG_COLOR = '#14100a';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'minimal';
const PB_KEY = 'mg_pb_rhythm-repeat';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Rhythm Master 🥁';
  if (acc >= 0.55) return 'Beat Keeper 🎵';
  if (sig.maxStreak >= 4) return 'Musical 🎶';
  return 'Off Beat 🎸';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Rhythm Repeat game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
