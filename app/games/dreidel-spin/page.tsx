...
const GAME_TAGLINE = 'Flick it hard. Watch it spin!';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Dreidel King 🌟';
  if (acc >= 0.55) return 'Spinner ✡️';
  if (sig.maxStreak >= 4) return 'Strong Flick 💪';
  return 'Shaky Spin 😬';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Dreidel Spin game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
