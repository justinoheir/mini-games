...
const GAME_TAGLINE = 'Dot dash. Fast. Go.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Telegraph Master ??';
  if (acc >= 0.55) return 'Code Tapper ���';
  if (sig.maxStreak >= 4) return 'Learning Morse ??';
  return 'Beep Boop ??';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Morse Tap game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
