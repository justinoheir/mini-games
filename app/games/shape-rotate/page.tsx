...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Spatial Genius 🌐';
  if (acc >= 0.55) return 'Mind Turner 🔄';
  if (sig.maxStreak >= 4) return 'Consistent 🎯';
  return 'Spatially Challenged 🧊';
}
...
export default function ShapeRotateGame() {
  ...
  const QUESTIONS: {q:string,opts:string[],c:number}[] = [{"q":"Triangle rotated 180° looks like?","opts":["Same","Flipped","Square","Circle"],"c":1},{"q":"L rotated 90° clockwise?","opts":["Γ","⌐","J","7"],"c":0},{"q":"Square at 45° looks like?","opts":["Circle","Diamond","Rectangle","Triangle"],"c":1},{"q":"3D cube rotated 90°?","opts":["Same cube","Different cube","Flat square","Sphere"],"c":1}];
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Shape Rotate game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
