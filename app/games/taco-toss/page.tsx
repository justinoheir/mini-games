...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Taco Chef 🌮';
  if (acc >= 0.55) return 'Taco Enthusiast 🫔';
  if (sig.maxStreak >= 4) return 'Ingredient Pro 👩‍🍳';
  return 'Taco Disaster 😂';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Taco Toss game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
