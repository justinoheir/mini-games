...
const GAME_TAGLINE = 'Sing it. Block it. Hold it.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Vocal Guardian 🛡️';
  if (acc >= 0.55) return 'Shield Singer 🎵';
  if (sig.maxStreak >= 4) return 'Sustained Voice 🔊';
...
export default function VocalShieldGame() {
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Vocal Shield game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
