...
const GAME_TAGLINE = 'Tilt to balance. Swipe for tricks.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Surf Pro 🏄';
  if (acc >= 0.55) return 'Wave Rider 🌊';
  if (sig.maxStreak >= 4) return 'Trick Artist ✨';
  return 'Wipeout Queen 💦';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Surf Ride game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
