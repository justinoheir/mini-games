...
const GAME_ID='category-clash';const ACCENT='#fb923c';const DURATION=45;const GAME_EMOJI='📦';const GAME_TITLE='Category Clash';const GAME_TAGLINE='Sort it fast. Categories clash!';
...
const COLORS=['#fb923c','#4ade80','#3b82f6','#f43f5e','#fbbf24','#a855f7'];
...
export default function CategoryClash(){
  ...
  return(
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      {phase==='start'&&<GameStartScreen emoji={GAME_EMOJI} title={GAME_TITLE} description="Swipe items into the right category before they disappear!" ctaLabel="Play! 📦" accentColor={theme.colors.accent??ACCENT} onStart={handleStart}/>}
      {phase==='countdown'&&<Countdown onComplete={startLoop} accentColor={theme.colors.accent??ACCENT}/>}
      {(phase==='playing'||phase==='countdown')&&(<><canvas ref={canvasRef} style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}} role="img" aria-label="Category Clash game canvas"/>
