...
const GAME_TAGLINE = 'Sort it fast. Categories clash!';
const BG_COLOR = '#14060a';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'minimal';
const PB_KEY = 'mg_pb_category-clash';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Sort Savant 🧠';
  if (acc >= 0.55) return 'Quick Sorter ⚡';
  if (sig.maxStreak >= 4) return 'Clash Champion 🏆';
  return 'Category Confused 🤷';
}
...
export default function CategoryClashGame() {
  ...
  const QUESTIONS: {q:string,opts:string[],c:number}[] = [{"q":"Fruits: Apple belongs in?","opts":["Fruit","Vegetable","Protein","Grain"],"c":0},{"q":"Soccer: Ball is?","opts":["Equipment","Clothing","Food","Vehicle"],"c":0},{"q":"Piano: It is a?","opts":["String","Wind","Percussion","Keyboard"],"c":3},{"q":"Dog: It is a?","opts":["Mammal","Bird","Fish","Reptile"],"c":0}];
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Category Clash game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
