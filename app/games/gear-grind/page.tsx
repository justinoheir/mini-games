...
const GAME_TAGLINE = 'Mesh the gears. Keep it spinning.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Engineer ⚙️';
  if (acc >= 0.55) return 'Mechanic 🔧';
  if (sig.maxStreak >= 4) return 'Grinder 💪';
  return 'Tinkerer 🔩';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Gear Grind game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
