...
const PB_KEY = 'mg_pb_sound-garden';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Garden Maestro 🌸';
  if (acc >= 0.55) return 'Green Thumb 🌱';
  if (sig.maxStreak >= 4) return 'Planter 🌿';
  return 'Seedling 🌾';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Sound Garden game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
