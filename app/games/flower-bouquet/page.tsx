...
const GAME_TAGLINE = 'Catch the petals. Build love.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Florist 💐';
  if (acc >= 0.55) return 'Gardener 🌸';
  if (sig.maxStreak >= 4) return 'Petal Collector 🌺';
  return 'Wilting 🥀';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Flower Bouquet game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
