...
const GAME_TAGLINE = 'Hold the chant. Charge the power.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Power Chanter 💥';
  if (acc >= 0.55) return 'Vocal Force ⚡';
  if (sig.maxStreak >= 4) return 'Sustained 🔋';
  return 'Whisper 🤫';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Chant Power game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
