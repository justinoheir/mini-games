...
const GAME_ID='odd-one-out';const ACCENT='#f97316';const DURATION=45;const GAME_EMOJI='🕵️';const GAME_TITLE='Odd One Out';const GAME_TAGLINE='Spot what doesn\x27t belong. Quick!';
...
const COLORS=['#f97316','#4ade80','#3b82f6','#f43f5e','#fbbf24','#a855f7'];
...
export default function OddOneOut(){
  ...
  return(
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      {phase==='start'&&<GameStartScreen emoji={GAME_EMOJI} title={GAME_TITLE} description="Find and tap the item that doesn\x27t belong in the group!" ctaLabel="Play! 🕵️" accentColor={theme.colors.accent??ACCENT} onStart={handleStart}/>}
      {phase==='countdown'&&<Countdown onComplete={startLoop} accentColor={theme.colors.accent??ACCENT}/>}
      {(phase==='playing'||phase==='countdown')&&(<><canvas ref={canvasRef} style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}} role="img" aria-label="Odd One Out game canvas"/>
