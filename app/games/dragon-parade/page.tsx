...
const GAME_TAGLINE = 'Multi-touch the dragon. Make it dance!';
const BG_COLOR = '#1a0000';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'holiday';
const PB_KEY = 'mg_pb_dragon-parade';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Parade Dragon 🐉';
  if (acc >= 0.55) return 'Dragon Dancer 🎊';
  if (sig.maxStreak >= 4) return 'Long Dragon 🌟';
  return 'Tangled Dragon 🪢';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Dragon Parade game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
