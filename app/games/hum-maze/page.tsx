...
const GAME_TAGLINE = 'Change your pitch. Navigate.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Voice Navigator 🗺️';
  if (acc >= 0.55) return 'Hum Pilot 🎵';
  if (sig.maxStreak >= 4) return 'Perseverant 💪';
  return 'Maze Humbler 🤔';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Hum Maze game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
