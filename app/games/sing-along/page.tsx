...
const GAME_TAGLINE = 'Match the note. Hold it perfect.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Soprano Star 🌟';
  if (acc >= 0.55) return 'On Key 🎵';
  if (sig.maxStreak >= 4) return 'In Tune 🎶';
  return 'Shower Singer 🚿';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Sing Along game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
