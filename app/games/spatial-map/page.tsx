...
const GAME_TAGLINE = 'Study the map. Answer fast.';
const BG_COLOR = '#000f14';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Human GPS 🗺️';
  if (acc >= 0.55) return 'Good Navigator 🧭';
  if (sig.maxStreak >= 4) return 'Directional 📍';
  return 'Lost Again 😅';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Spatial Map game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
