...
const BG_COLOR = '#1a0014';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'sports';
const PB_KEY = 'mg_pb_gymnast-beam';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Gold Medalist 🥇';
  if (acc >= 0.55) return 'Gymnast 🤸';
  if (sig.maxStreak >= 4) return 'Balanced ⚖️';
  return 'Falling Star 💫';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Gymnast Beam game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
