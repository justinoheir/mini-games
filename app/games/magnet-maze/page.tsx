...
const GAME_TAGLINE = 'Attract, repel, navigate.';
const BG_COLOR = '#0a0000';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Navigator 🧭';
  if (acc >= 0.55) return 'Pathfinder 🗺️';
  if (sig.maxStreak >= 4) return 'Careful 🐢';
  return 'Lost 😵';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Magnet Maze game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
