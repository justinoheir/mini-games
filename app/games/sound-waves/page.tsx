...
const GAME_TAGLINE = 'Shout the frequency. Shatter walls.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Sonic Boom 💥';
  if (acc >= 0.55) return 'Wave Rider 🌊';
  if (sig.maxStreak >= 4) return 'Loud and Proud 📢';
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Sound Waves game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
