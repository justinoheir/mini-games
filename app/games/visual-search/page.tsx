...
const BG_COLOR = '#001207';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'minimal';
const PB_KEY = 'mg_pb_visual-search';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Eagle Eye 🦅';
  if (acc >= 0.55) return 'Hunter 🎯';
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Visual Search game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
