...
const GAME_ID='visual-search';const ACCENT='#10b981';const DURATION=45;const GAME_EMOJI='🔍';const GAME_TITLE='Visual Search';const GAME_TAGLINE='Find it. Tap it. Before the horde.';
...
const COLORS=['#10b981','#4ade80','#3b82f6','#f43f5e','#fbbf24','#a855f7'];
...
export default function VisualSearch(){
  ...
  return(
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      {phase==='start'&&<GameStartScreen emoji={GAME_EMOJI} title={GAME_TITLE} description="Tap the target item hidden among distractors!" ctaLabel="Play! 🔍" accentColor={theme.colors.accent??ACCENT} onStart={handleStart}/>}
      {phase==='countdown'&&<Countdown onComplete={startLoop} accentColor={theme.colors.accent??ACCENT}/>}
      {(phase==='playing'||phase==='countdown')&&(<><canvas ref={canvasRef} style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}} role="img" aria-label="Visual Search game canvas"/>
