...
const GAME_TAGLINE = 'Sweep it in. Sweep it hard.';
const BG_COLOR = '#001a1f';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Skip Champion 🥌';
  if (acc >= 0.55) return 'Ice Master ❄️';
  if (sig.maxStreak >= 4) return 'Sweeper 🧹';
  return 'Ice Rookie 🧊';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Curling Sweep game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
