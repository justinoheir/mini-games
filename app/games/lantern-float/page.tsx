...
const GAME_TAGLINE = 'Blow them up. Watch them rise.';
const BG_COLOR = '#14060a';
const MUSIC_PAT: import('@/lib/audio').MusicPattern = 'holiday';
const PB_KEY = 'mg_pb_lantern-float';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Sky Lantern 🏮';
  if (acc >= 0.55) return 'Float Master 🕯️';
  if (sig.maxStreak >= 4) return 'High Blower 🌬️';
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Lantern Float game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
