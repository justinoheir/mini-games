...
const GAME_TAGLINE = 'Breathe to shape. Slow or fast.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Breath Artist 🌬️';
  if (acc >= 0.55) return 'Breath Control 🧘';
  if (sig.maxStreak >= 4) return 'Sculptor ✨';
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Breath Sculpt game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
