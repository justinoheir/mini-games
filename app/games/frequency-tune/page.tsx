...
const GAME_TAGLINE = 'Find the frequency. Hold it.';
...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Perfect Pitch 🎼';
  if (acc >= 0.55) return 'Frequency Finder 📻';
  if (sig.maxStreak >= 4) return 'Patient Tuner ⏱️';
  return 'Off Frequency 📡';
}
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Frequency Tune game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
