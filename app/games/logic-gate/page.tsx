...
function getPersonality(sig: Signals): string {
  ...
  if (acc >= 0.75 && avg < 700) return 'Hardware Engineer 💻';
  if (acc >= 0.55) return 'Logic Master 🔌';
  if (sig.maxStreak >= 4) return 'Systematic ⚙️';
...
export default function LogicGateGame() {
  ...
  const QUESTIONS: {q:string,opts:string[],c:number}[] = [{"q":"AND gate: inputs 1 and 0. Output?","opts":["0","1","Both","Error"],"c":0},{"q":"OR gate: inputs 0 and 0. Output?","opts":["0","1","Error","Undefined"],"c":0},{"q":"NOT gate: input 1. Output?","opts":["0","1","2","Null"],"c":0},{"q":"AND gate: inputs 1 and 1. Output?","opts":["0","1","2","Error"],"c":1}];
  ...
  return (
    <GameShell title={GAME_TITLE} emoji={GAME_EMOJI} accentColor={theme.colors.accent??ACCENT}>
      ...
      {(phase==='playing'||phase==='countdown')&&<>
        <canvas ref={canvasRef} aria-label="Logic Gate game canvas" role="img" style={{position:'absolute',inset:0,width:'100%',height:'100%',touchAction:'none'}}/>
